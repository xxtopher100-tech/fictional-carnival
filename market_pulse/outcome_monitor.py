...
def _process_one_trade(
    ...
    # Live price: crypto via get_best_price; FX pairs via forex rate helper
    price, _ = get_best_price(coin)
    if (not price or price <= 0) and coin and ("/" in str(coin) or str(coin).upper() in ("EUR", "GBP")):
        try:
            from market_pulse.forex_trade_engine import get_forex_rate
            pair = str(coin).upper().strip()
            if pair in ("EUR", "BUY EUR"):
                pair = "EUR/USD"
            elif pair in ("GBP", "BUY GBP"):
                pair = "GBP/USD"
            rate, _, _, _ = get_forex_rate(pair)
            if rate and float(rate) > 0:
                price = float(rate)
                logger.debug("[OUTCOME] #%s FX price %s=%s", idea_id, pair, price)
        except Exception as _fx_e:
            logger.debug("[OUTCOME] #%s FX price fallback: %s", idea_id, _fx_e)
    if not price or price <= 0:
        # Suppressed / unpriceable rows: expire when past valid_until so they leave the open queue
        pub = (publication_status or "").upper().strip()
        vu_chk = (valid_until or "").strip()
        if not vu_chk and created_at:
            try:
                hz = resolve_horizon(timeframe, tier)
                vu_chk = compute_valid_until(created_at, hz["valid_hours"])
            except Exception:
                vu_chk = ""
        past = False
        if vu_chk:
            try:
                past = now > datetime.strptime(vu_chk[:19], "%Y-%m-%d %H:%M:%S")
            except Exception:
                past = False
        if past or pub in ("SUPPRESSED", "PUBLISH_FAILED"):
            try:
                c.execute(
                    """
                    UPDATE trade_ideas
                    SET status='closed', result='EXPIRED', expired_at=%s, closed_at=%s,
                        lifecycle_status='EXPIRED',
                        last_notified_state=COALESCE(NULLIF(last_notified_state,''), 'EXPIRED')
                    WHERE id=%s AND status='open'
                    """,
                    (now_s, now_s, idea_id),
                )
                logger.info(
                    "[OUTCOME] #%s %s closed EXPIRED (no price; pub=%s)",
                    idea_id, coin, pub or "n/a",
                )
            except Exception as _ex:
                logger.debug("[OUTCOME] expire no-price #%s: %s", idea_id, _ex)
        else:
    ...
    if new_state in ("ACTIVE", "STILL_OPEN", "ENTRY_NOT_REACHED", "", "None"):
        ...
        if after:
            try:
                highs = [float(c.get("high") or 0) for c in after if c.get("high") is not None]
                lows = [float(c.get("low") or 0) for c in after if c.get("low") is not None]
                if highs:
                    hi = max(highs)
                if lows:
                    lo = min(lows)
            ...
            if is_long:
                ...
                if entry_seen or (px is not None and px >= en) or (lo is not None and lo <= en):
                    if st is not None and lo is not None and lo <= st:
                        new_state = "STOP_HIT"
                    elif st is not None and px is not None and px <= st:
                        new_state = "STOP_HIT"
                    elif tp2 is not None and hi is not None and hi >= tp2:
                        new_state = "TP2_HIT"
                ...
                if entry_seen or (px is not None and px <= en) or (hi is not None and hi >= en):
                    if st is not None and hi is not None and hi >= st:
                        new_state = "STOP_HIT"
                    elif st is not None and px is not None and px >= st:
                        new_state = "STOP_HIT"
                    elif tp2 is not None and lo is not None and lo <= tp2:
                        new_state = "TP2_HIT"
                    elif tp2 is not None and px is not None and px <= tp2:
                        new_state = "TP2_HIT"
