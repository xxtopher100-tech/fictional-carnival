...
def _programmatic_forex_levels(pair_key, rate, tier):
    ...
    # Floor stop distance so we never publish "stop ≈ entry" noise (seen in live stops).
    vol = max(float(pair.get("typical_spread") or 0), rate * 0.002)
    min_stop = rate * 0.0025  # ≥0.25% of rate (tradeable room on majors)
    stop_dist = max(min_stop, min(vol * 3.0, rate * max_stop_pct * 0.9))
    if stop_dist <= 0:
        return None
...
def _validate_forex_trade(pair_key, rate, trade, tier="momentum"):
    ...
    # Minimum risk distance — reject stop≈entry / pure spread noise
    risk_pct = (risk / entry) * 100.0
    if risk_pct < 0.25:
        return False, f"Stop too tight ({risk_pct:.3f}% < 0.25%)"
    # Absolute floor for USD majors (~15–20 pips on EUR/GBP)
    if entry < 5 and risk < 0.0015:
        return False, f"Stop distance {risk:.5f} below 0.0015 major-FX floor"
    ...
    if risk_pct > max_stop_pct:
        return False, f"Stop too wide ({risk_pct:.1f}% > {max_stop_pct}%)"
    ...
    try:
        ...
        if trade:
            try:
                ...
                if ai_raw:
                    ai_clean = sanitize_ai_narrative(ai_raw.strip()[:400], fallback="")
                    if ai_clean:
                        trade["rationale"] = (ai_clean + "\n\n" + (trade.get("rationale") or "")).strip()
            except Exception:
                pass
        else:
