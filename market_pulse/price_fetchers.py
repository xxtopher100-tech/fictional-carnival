...
_fiat_fetch_lock = threading.Lock()
_FIAT_CACHE_TTL_SECONDS = 14400  # 4 hours — frankfurter rates move slowly
...
def get_fiat_rates():
    ...
    Sources: frankfurter.app (primary) + open.er-api.com (fallback).
    NGN is derived from Binance/Bybit P2P when available.

    IMPORTANT: Cache is written *before* the optional NGN P2P derivation.
    get_p2p_rate() falls back to get_fiat_rates() when exchange P2P is
    blocked — without an early cache write that created a re-entrant
    frankfurter hammer loop (logs every 0.5–1s during startup).
    """
    global _fiat_cache
    now = wat_now()

    # Fast path: serve warm cache without taking the lock
    if (_fiat_cache["timestamp"] and
            (now - _fiat_cache["timestamp"]).total_seconds() < _FIAT_CACHE_TTL_SECONDS):
        return _fiat_cache["data"]

    with _fiat_fetch_lock:
        # Re-check after acquiring lock (another thread may have filled cache)
        now = wat_now()
        if (_fiat_cache["timestamp"] and
                (now - _fiat_cache["timestamp"]).total_seconds() < _FIAT_CACHE_TTL_SECONDS):
            return _fiat_cache["data"]
        ...
        # Cache major rates BEFORE any call that might re-enter get_fiat_rates
        # (get_p2p_rate → get_fiat_rates when Binance/Bybit P2P is geo-blocked).
        if rates:
            _fiat_cache["data"] = rates
            _fiat_cache["timestamp"] = now

        # ── NGN enrichment (frankfurter excludes NGN) ───────────────────
        # Order: open.er-api (has NGN) → P2P mid → last-resort constant.
        # Never blocks cache of other fiat rates.
        if rates and "NGN" not in rates:
            try:
                resp = fetch_with_backoff("https://open.er-api.com/v6/latest/USD")
                if resp and "rates" in resp and resp["rates"].get("NGN"):
                    rates["NGN"] = round(float(resp["rates"]["NGN"]), 2)
                    logger.info("[FIAT RATES] NGN from er-api: ₦%.0f", rates["NGN"])
            except Exception as e:
                logger.debug("[FIAT RATES] NGN er-api: %s", e)
        if rates and "NGN" not in rates:
            try:
                from market_pulse.p2p import get_binance_p2p, get_bybit_p2p
                buy_ngn = get_binance_p2p("BUY", "USDT", "NGN")
                sell_ngn = get_binance_p2p("SELL", "USDT", "NGN")
                source = "Binance P2P"
                if not (buy_ngn and sell_ngn):
                    buy_ngn = get_bybit_p2p("BUY", "USDT", "NGN")
                    sell_ngn = get_bybit_p2p("SELL", "USDT", "NGN")
                    source = "Bybit P2P"
                if buy_ngn and sell_ngn:
                    rates["NGN"] = round((buy_ngn + sell_ngn) / 2, 2)
                    logger.info("[FIAT RATES] NGN from %s: ₦%.0f", source, rates["NGN"])
            except Exception as e:
                logger.warning("[FIAT RATES] NGN P2P derivation: %s", e)
        if rates and "NGN" not in rates:
            # Emergency floor so P2P estimate path never fully dies
            rates["NGN"] = 1600.0
            logger.warning("[FIAT RATES] NGN fallback constant ₦1600 (update when P2P works)")
