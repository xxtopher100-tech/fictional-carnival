...
from market_pulse.ai_trade_setup_rr import (
    _ts_parse_price_token,
    _ts_extract_trade_levels,
    _ts_calc_rr,
    _ts_format_rr,
    _ts_strip_ai_rr_claims,
    _ts_build_rr_section,
)
from market_pulse.schedule_locks import (
    _schedule_lock_key,
    _schedule_already_posted,
    _schedule_clear_posted,
    ...
    while True:
        try:
            ...
            if now - last_outcome_score >= 300:
                ...
                # Legacy outcome/shadow optional via env
                import os as _os
                if _os.environ.get("LEGACY_OUTCOME", "0").strip() in ("1", "true", "yes"):
                    try:
                        threading.Thread(
                ...
                if _os.environ.get("LEGACY_SHADOW", "0").strip() in ("1", "true", "yes"):
                    try:
                        threading.Thread(
